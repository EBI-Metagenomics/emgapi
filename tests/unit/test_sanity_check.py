...
def gen_all_flag_files(tmpdir):
    filelist = sanity_check.AmpliconSanityCheck.flag_files
    for f in filelist:
...
def gen_all_qc_files(tmpdir):
    ...
    filelist = sanity_check.AmpliconSanityCheck.qc_files
    for f in filelist:
        write_file(qc_dir / f, 'data')


def gen_all_maindir_files(tmpdir, prefix):
    filelist = sanity_check.AmpliconSanityCheck.result_files
...
def gen_all_seqdir_files(tmpdir):
    ...
    seq_dir.mkdir()
    filelist = sanity_check.AmpliconSanityCheck.seq_categorisation
    for f in filelist:
