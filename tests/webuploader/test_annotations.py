...
class TestAnnotations:
    ...
    def test_ipr(self, client, run):
        ...
        url = reverse("emgapi_v1:interproidentifier-detail", args=["IPR009739"])
        response = client.get(url)
        assert response.status_code == status.HTTP_200_OK
        rsp = response.json()
    ...
    def test_kegg_modules(self, client, analysis_results, pipeline_version, expected):
        ...
        for entry in rsp["data"]:
            ...
            for key in detail_data["data"]["attributes"].keys():
                if key != "accession":
                    assert (
                        detail_data["data"]["attributes"][key]
                        == expected["data"][accession][key]
                    )
    ...
    def test_genome_properties(
        self, client, analysis_results, pipeline_version, expected
    ):
        ...
        [
            {
                "data": {
                    ...
                    "bacteriocin": [
                        11,
                        "Bacteriocin or other unspecified "
    ...
    def test_antismash(self, client, analysis_results, pipeline_version, expected):
        ...
        url = reverse(
            "emgapi_v1:analysis-antismash-gene-clusters-list", args=[job.accession]
        )
        response = client.get(url)
        ...
        for entry in rsp["data"]:
            ...
            detail_url = reverse(
                "emgapi_v1:antismash-gene-clusters-detail", args=[accession]
            )
            detail_resp = client.get(detail_url)
            assert detail_resp.status_code == status.HTTP_200_OK
            detail_data = detail_resp.json()
            ...
            assert (
                detail_data["data"]["attributes"]["description"]
                == expected["data"][accession][1]
            )
