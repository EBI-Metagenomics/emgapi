...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        while offset < runs_count:
            emg_runs_batch = emg_models.Run.all()[offset:batch_size]
            ena_runs_batch = ena_models.Run.filter(
                run_id__in=[run.accession for run in emg_runs_batch]
            )

            for emg_run in emg_runs_batch:
                ena_run = next(
                    (el for el in ena_studies_batch if el.run_id == emg_run.accession),
                    default=None,
                )
                if ena_run is None:
                    logger.error(f"{emg_run} not found in ENA.")
                    continue
                if ena_run.status is None:
                    logger.error(f"{emg_run} on ENA has no value on the column status.")
                    continue
                emg_run.sync_with_ena_status(ena_run.status)
                emg_run.public_release_date = ena_hold_date

            emg_models.Run.objects.bulk_update(
                emg_runs_batch, ["is_private", "is_suppressed", "reason"]
            )
            logger.info(f"Batch {runs_count / batch_size::.0f} processed.")
            offset += batch_size
